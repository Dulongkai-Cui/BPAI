...
type OnlyOfficeSlideEditorProps = {
...
export function OnlyOfficeSlideEditor({
  documentTitle,
  documentKey,
  documentUrl,
  fileType,
    ...
    const mountEditor = () => {
      ...
      try {
        editorInstanceRef.current = new window.DocsAPI.DocEditor(
          "bpai-onlyoffice-slide-editor",
          editorConfig,
        );
        setLoadError(null);
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "ONLYOFFICE 演示文稿编辑器初始化失败";
  ...
  return (
    <div
      className={`flex h-full min-h-0 flex-col overflow-hidden ${
        minimal ? "bg-white" : "rounded-[28px] border border-slate-200 bg-[#fff5eb] p-3 shadow-inner"
      }`}
    >
      {minimal ? null : (
      ...
      <div
        className={`min-h-0 flex-1 overflow-hidden ${
          minimal
            ? "bg-white"
            : "rounded-[24px] border border-slate-200 bg-[#f8fafc] p-3 shadow-[0_24px_60px_rgba(15,23,42,0.08)]"
        }`}
      >
        <div
          className={
            minimal
              ...
              : "h-full overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.12)]"
          }
        >
          {loadError ? (
            <div className="flex h-full items-center justify-center bg-white p-6 text-center text-sm text-slate-600">
              <div>
                <div className="font-semibold text-slate-900">ONLYOFFICE 演示文稿暂未成功加载</div>
          ...
          <div
            id="bpai-onlyoffice-slide-editor"
