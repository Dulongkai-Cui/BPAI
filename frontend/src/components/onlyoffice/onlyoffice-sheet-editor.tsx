...
type OnlyOfficeSheetEditorProps = {
...
export function OnlyOfficeSheetEditor({
  documentTitle,
  documentKey,
  documentUrl,
  fileType,
  minimal = false,
}: OnlyOfficeSheetEditorProps) {
    ...
    const mountEditor = () => {
      ...
      try {
        editorInstanceRef.current = new window.DocsAPI.DocEditor(
          "bpai-onlyoffice-sheet-editor",
          editorConfig,
        );
        setLoadError(null);
      } catch (error) {
        const message = error instanceof Error ? error.message : "ONLYOFFICE 表格编辑器初始化失败";
  ...
  return (
    <div className={`flex h-full min-h-0 flex-col overflow-hidden ${minimal ? "bg-white" : "rounded-[28px] border border-slate-200 bg-[#edf7f1] p-3 shadow-inner"}`}>
      {minimal ? null : (
        <div className="mb-3 flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400">BPAI Sheets</div>
            <div className="mt-1 text-sm font-semibold text-slate-900">ONLYOFFICE 在线表格编辑器</div>
          </div>
          <div className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700">Excel-like Workspace</div>
        </div>
      )}
        ...
        <div className={minimal ? "h-full overflow-hidden bg-white" : "h-full overflow-hidden rounded-[20px] border border-slate-200 bg-white shadow-[0_24px_80px_rgba(15,23,42,0.12)]"}>
          {loadError ? (
            <div className="flex h-full items-center justify-center bg-white p-6 text-center text-sm text-slate-600">
              <div>
                <div className="font-semibold text-slate-900">ONLYOFFICE 表格暂未成功加载</div>
                <p className="mt-2 leading-6">{loadError}</p>
                <p className="mt-2 leading-6 text-slate-500">
                  请确认 Document Server 已启动，并可访问：{scriptSrc}
                </p>
                <p className="mt-2 break-all text-xs leading-5 text-slate-400">当前表格地址：{documentUrl}</p>
              </div>
            </div>
          ) : null}
          <div
            id="bpai-onlyoffice-sheet-editor"
