...
type BpAskTopbarProps = {
  user: AuthenticatedUser;
};

export function BpAskTopbar({ user }: BpAskTopbarProps) {
  ...
  return (
    <header className="fixed top-0 left-0 z-40 flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white px-6 shadow-sm">
      <div className="flex items-center gap-4">
        <span className="text-[22px] font-bold tracking-tight text-blue-700">
          我的BPAI
        </span>
      ...
      <div className="flex items-center gap-4">
        <div className="relative hidden sm:block">
          ...
          <input
            type="text"
            placeholder="搜索对话、工单或系统表单..."
            className="w-72 rounded-full border border-slate-200 bg-slate-50 py-1.5 pr-4 pl-10 text-sm text-slate-700 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
          />
        </div>
